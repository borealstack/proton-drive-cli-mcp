...
1. Start with `proton_drive_auth_status` when the user's auth state is unknown.
2. If the CLI is missing, the MCP server auto-installs the latest official Proton Drive CLI for the current OS/CPU and verifies Proton's SHA-512 checksum before use.
...
- `proton_drive_list` for browsing folders.
