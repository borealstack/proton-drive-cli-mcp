...
This Codex plugin starts the Proton Drive CLI MCP server package.

## What It Provides

- MCP tools for Proton Drive auth status, browser login, file operations, sharing, and invitations.
...
The CLI path usually should not be hard-coded; set `PROTON_DRIVE_CLI_PATH` only when you need to pin a specific binary.
...
If the CLI is missing, the MCP server downloads the latest official binary for the current OS/CPU, verifies Proton's SHA-512 checksum, and installs it to a user-local managed path. Set `PROTON_DRIVE_CLI_AUTO_INSTALL=0` to disable this.
